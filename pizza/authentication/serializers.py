...
class UserCreationSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        email = MyUser.objects.filter(username=attrs.get('username')).exists()

        if email:
            raise ValidationError(detail="User with email exists", code=status.HTTP_403_FORBIDDEN)

        username = MyUser.objects.filter(username=attrs.get('username')).exists()
    ...
    def create(self, validated_data):
        new_user = MyUser(**validated_data)
